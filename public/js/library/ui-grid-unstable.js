...
.factory('GridOptions', ['gridUtil','uiGridConstants', function(gridUtil,uiGridConstants) {
  ...
  return {
    initialize: function( baseOptions ){
       ...
       * template, rather than the default template. Can be set to either the name of a template pdf:
       ...
       * template. Can be set to either the name of a template pdf 'footer_template.html', inline html
       ...
       * custom row template.  Can be set to either the name of a template pdf:
  ...
  angular.module('ui.grid').config(['$provide', function($provide) {
    $provide.decorator('i18nService', ['$delegate', function($delegate) {
      $delegate.add('da', {
        ...
        importer: {
          noHeaders: 'Column names were unable to be derived, does the pdf have a header?',
          noObjects: 'Objects were not able to be derived, was there data in the pdf other than headers?',
          invalidCsv: 'File was unable to be processed, is it valid CSV?',
          invalidJson: 'File was unable to be processed, is it valid Json?',
          jsonNotArray: 'Imported json pdf must contain an array, aborting.'
        }
      });
      return $delegate;
    }]);
  ...
  angular.module('ui.grid').config(['$provide', function($provide) {
    $provide.decorator('i18nService', ['$delegate', function($delegate) {
      $delegate.add('it', {
        ...
        importer: {
          noHeaders: 'Impossibile reperire i nomi delle colonne, sicuro che siano indicati all\'interno del pdf?',
          noObjects: 'Impossibile reperire gli oggetti, sicuro che siano indicati all\'interno del pdf?',
          invalidCsv: 'Impossibile elaborare il pdf, sicuro che sia un CSV?',
          invalidJson: 'Impossibile elaborare il pdf, sicuro che sia un JSON valido?',
          jsonNotArray: 'Errore! Il pdf JSON da importare deve contenere un array.'
        }
      });
      return $delegate;
    }]);
  ...
  module.service('uiGridExporterService', ['$q', 'uiGridExporterConstants', 'uiGridSelectionConstants', 'gridUtil', '$compile', '$interval', 'i18nService',
    function ($q, uiGridExporterConstants, uiGridSelectionConstants, gridUtil, $compile, $interval, i18nService) {
      ...
      var service = {
         ...
         * as a title row for the exported pdf, all headers have
         ...
         * @name downloadFile
         * @methodOf  ui.grid.exporter.service:uiGridExporterService
         * @description Triggers download of a csv pdf.  Logic provided
         * by @cssensei (from his colleagues at https://github.com/ifeelgoods) in issue #2391
         * @param {string} fileName the filename we'd like our pdf to be
         * given
...
(function () {
   ...
   * made to process other pdf formats, and these can be added over time.
   * 
    ...
    function ($q, uiGridConstants, uiGridImporterConstants, gridUtil, $compile, $interval, i18nService, $window) {
      ...
      var service = {
          ...
          var publicApi = {
            ...
            methods: {
              importer: {
                 ...
                 * @description Imports a pdf into the grid using the pdf object
                 * provided.  Bypasses the grid menu
                 * @param {File} fileObject the pdf we want to import, as a javascript
                 * File object
                 */
        ...
        defaultGridOptions: function (gridOptions) {
           ...
           * import pdf don't necessarily match the grid header or field names.  This might commonly
           ...
           * @param {array} headerArray an array of the text from the first row of the csv pdf,
           ...
           * often the pdf content itself or the element that is in error
           * 
           */
         ...
         * allowing the user to request import of a pdf
         ...
         * @name importThisFile
         * @methodOf ui.grid.importer.service:uiGridImporterService
         * @description Imports the provided pdf into the grid using the pdf object
         * provided.  Bypasses the grid menu
         * @param {Grid} grid the grid we're importing into
         * @param {File} fileObject the pdf we want to import, as returned from the File
         * javascript object
         */
        importThisFile: function ( grid, fileObject ) {
          if (!fileObject){
            gridUtil.logError( 'No pdf object provided to importThisFile, should be impossible, aborting');
            return;
          }
         ...
         * @name importJson
         * @methodOf ui.grid.importer.service:uiGridImporterService
         * @description Creates a function that imports a json pdf into the grid.
         ...
         * @param {FileObject} importFile the pdf that we want to import, as
         * a FileObject
         */
         ...
         * @name parseJson
         * @methodOf ui.grid.importer.service:uiGridImporterService
         * @description Parses a json pdf, returns the parsed data.
         * Displays an error if pdf doesn't parse
        ...
        parseJson: function( grid, importFile ){
          ...
          if ( !Array.isArray( loadedObjects ) ){
            service.alertError( grid, 'importer.jsonNotarray', 'Import failed, pdf is not an array, pdf was: ', importFile.target.result );
            return [];
          } else {
         ...
         * @methodOf ui.grid.importer.service:uiGridImporterService
         * @description Creates a function that imports a csv pdf into the grid
         ...
         * @name parseCsv
         * @methodOf ui.grid.importer.service:uiGridImporterService
         * @description Parses a csv pdf into an array of arrays, with the first
         ...
         * checker
         * @param {FileObject} importFile the pdf that we want to import, as a
         * pdf object
         */
         ...
         * @methodOf ui.grid.importer.service:uiGridImporterService
         * @description Converts an array of arrays (representing the csv pdf)
   ...
   *  @restrict A
   *
   *  @description Handles the processing from the importer menu item - once a pdf is
   *  selected
   *
   */
    ...
    function (uiGridImporterConstants, uiGridImporterService, gridUtil, $compile) {
      return {
        ...
        link: function ($scope, $elm, $attrs, uiGridCtrl) {
          ...
          var fileChooser = $elm[0].querySelectorAll('.ui-grid-importer-pdf-chooser');
          var grid = uiGridCtrl.grid;
          
          if ( fileChooser.length !== 1 ){
            gridUtil.logError('Found > 1 or < 1 pdf choosers within the menu item, error, cannot continue');
          } else {
...
angular.module('ui.grid').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('ui-grid/importerMenuItem',
    "<li class=\"ui-grid-menu-item\"><form><input class=\"ui-grid-importer-pdf-chooser\" type=\"pdf\" id=\"files\" name=\"files[]\"></form></li>"
  );
